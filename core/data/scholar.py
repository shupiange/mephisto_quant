...
class Scholar:
    ...
    def _get_db(self):
        if self.db is None:
            self.db = MySQLManager(**self.db_config)
        self.db.connect()
        return self.db
    ...
    def load_stock_batch_data(self, codes):
        ...
        if '30_minute' in self.source_table:
            ...
            select_cols = "code, date, time, open, close, high, low, volume, amount"
            df_cols = ['code', 'date', 'time', 'open', 'close', 'high', 'low', 'volume', 'amount']
        else:
            order_by = "ORDER BY code, date"
            select_cols = "code, date, open, close, high, low, volume, amount"
            df_cols = ['code', 'date', 'open', 'close', 'high', 'low', 'volume', 'amount']
    ...
    def save_indicators_batch(self, df):
        ...
        if '30_minute' in self.target_table:
            df_save = df[['date', 'time', 'code'] + indicator_cols].copy()
        else:
            df_save = df[['date', 'code'] + indicator_cols].copy()

        df_save = df_save.replace([np.inf, -np.inf], np.nan)
    ...
    def run(self):
        """主流程"""
        print(f"\n{'='*60}")
        print(f"Scholar - Technical Indicators Calculator")
