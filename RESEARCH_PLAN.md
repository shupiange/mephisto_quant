...
当前研究默认以第三版策略 `relative_strength_pullback_daily` 为基准，而不是继续使用最早的 `trend_macd_daily`。

这样调整的原因是：

- 第一版“趋势 + MACD”更像验证回测链路是否可用的基线策略
- 第二版已经开始聚焦“前强 + 回调 + 启动”的个股形态
- 第三版进一步加入“全市场横截面相对强势”的筛选，更接近“强势板块里的强势股”的代理方案
- 在当前尚未补齐行业/概念/板块映射数据之前，第三版是最贴近实盘思路的默认研究版本

#### 当前默认策略名称

`relative_strength_pullback_daily`

#### 当前默认策略假设

只做“中期已经强过、短期仍保持强势、随后回调到均线附近、再度放量启动”的股票。  
在板块/行业强度数据还未完备前，先用**全市场横截面的相对强弱排序**来代理“强势板块里的强势股”。

#### 股票池

- 全 A 股中剔除未上市、已退市或状态异常样本
- 当日成交额 `amount` 达到流动性门槛
- `close > ma60`
- `ma20 > ma60`
- `macd > 0`
...
1. 在 `prior_strength_lookback` 窗口内，个股曾经出现一段明显上涨（前强）
2. 在 `short_strength_lookback` 窗口内，个股首尾收益率仍保持为正（短强）
3. 在候选股之间继续按得分排序，只保留横截面中更强的一部分股票

#### 回调与启动规则

满足以下条件后，于下一交易日开盘买入：

1. 当前价格相对近期高点已有一定回落，但回撤不能过深
2. 当前价格稳定在 `ma20` 附近
3. 最近几天有回踩 `ma20` 的动作
4. 当日 `close > open`
5. `close >= ma20`
...
- 最大持仓数：由研究参数控制
- 单票仓位上限：由 `position_pct` 控制
- 组合回撤限制：由 `max_drawdown_pct` 控制
- 保留 T+1 交收约束，不做日内回转
- 策略内负责主要退出逻辑，研究入口只保留组合级风控与仓位约束

#### 当前版本交付物

| 文件 | 内容 |
|------|------|
| `core/strategy/relative_strength_pullback_daily.py` | 当前默认研究策略 |
| `core/strategy/pullback_breakout_daily.py` | 第二版策略，对照“前强 + 回调 + 启动” |
| `core/strategy/trend_macd_daily.py` | 第一版基线策略，对照“趋势 + MACD” |
| `run_research.py` | 统一研究入口 |
| `research_outputs/` | 每次研究的净值、交易、摘要与研究报告 |

#### 当前验收标准

- 小样本调试可正常产生交易，且买卖方向符合预期
- 全量回测可稳定跑完，无字段缺失或空股票池报错
- 输出收益、回撤、夏普、胜率、盈亏比五项核心指标
- 可与第一版、第二版策略做横向对照
- 后续在接入板块/行业映射数据后，可平滑升级到真正的“强势板块 + 强势股 + 回调启动”方案
